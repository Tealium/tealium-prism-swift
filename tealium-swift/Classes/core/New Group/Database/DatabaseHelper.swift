...
public class DatabaseHelper: SQLiteOpenHelper {
    ...
    internal static var databaseUpgrades: [DatabaseUpgrade] = [
    ]
    
    internal static var DATABASE_VERSION = 1
    ...
    public override func onCreate(database: Connection) throws {
        // Create Queue Tables
        try DispatchSchema.createtable(db: database)
        try DispatcherSchema.createTable(db: database)
        try QueueSchema.createTable(db: database)
        
        // Create Module Tables
        try ModuleSchema.createTable(db: database)
        try ModuleStorageSchema.createTable(db: database)
