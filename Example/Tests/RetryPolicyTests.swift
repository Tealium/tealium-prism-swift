...
final class RetryPolicyTests: XCTestCase {
    ...
    func test_after_event_subscribes_once_on_queue() {
        ...
        let policy = RetryPolicy.afterEvent(TealiumObservableCreate({ observer in
...
final class DelayPolicyTests: XCTestCase {
    ...
    func test_do_not_delay_should_never_delay() {
        let policy = DelayPolicy.doNotDelay
        let expect = expectation(description: "Retry should never happen")
        expect.isInverted = true
        let result = policy.shouldDelay(onQueue: .main) {
            expect.fulfill()
        }
        XCTAssertFalse(result)
        waitForExpectations(timeout: 3.0)
    ...
    func test_after_delay_should_delay_once_on_queue() {
        let policy = DelayPolicy.afterDelay(0.0)
        let expect = expectation(description: "Retry should happen once")
        let queue = DispatchQueue(label: "test_label")
        let result = policy.shouldDelay(onQueue: queue) {
            dispatchPrecondition(condition: .onQueue(queue))
            expect.fulfill()
        }
        XCTAssertTrue(result)
    ...
    func test_after_event_subscribes_once_on_queue() {
        ...
        let otherQueue = DispatchQueue(label: "other_label")
        let expect = expectation(description: "Retry should subscribe once")
        let policy = DelayPolicy.afterEvent(TealiumObservableCreate({ observer in
            defer { expect.fulfill() }
            dispatchPrecondition(condition: .onQueue(queue))
            otherQueue.async {
                observer(())
    ...
    func test_after_event_should_delay_once_on_queue() {
        let queue = DispatchQueue(label: "queue_label")
        let otherQueue = DispatchQueue(label: "other_label")
        let policy = DelayPolicy.afterEvent(TealiumObservableCreate({ observer in
            dispatchPrecondition(condition: .onQueue(queue))
