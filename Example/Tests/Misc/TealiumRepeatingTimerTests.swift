...
final class TealiumRepeatingTimerTests: XCTestCase {
    func test_timer_calls_eventHandler() {
        ...
        let timer = TealiumRepeatingTimer(timeInterval: 0.01, repeating: .never, queue: .main, eventHandler: {
    ...
    func test_timer_repeats_by_default() {
        ...
        let timer = TealiumRepeatingTimer(timeInterval: 0.01, queue: .main, eventHandler: {
            timerCompleted.fulfill()
        })
        timer.resume()
        waitForDefaultTimeout()
    ...
    func test_timer_sets_repeat_with_timeInterval_by_default() {
        let timer = TealiumRepeatingTimer(timeInterval: 0.13, queue: .main, eventHandler: { })
    ...
    func test_timer_repeats_with_repeating_value_when_provided() {
        ...
        let timer = TealiumRepeatingTimer(timeInterval: 0.01, repeating: .milliseconds(200), queue: .main, eventHandler: {
    ...
    func test_timer_can_be_deinitialized() {
        ...
        let helper = RetainCycleHelper(variable: TealiumRepeatingTimer(timeInterval: 0.1, repeating: .milliseconds(1), queue: .main, eventHandler: {
