...
final class NetworkHelperTests: XCTestCase {
    ...
    func test_getJsonAsObject_returns_codable_result() {
        ...
        mockClient.result = .success(NetworkResponse(data: try! JSONEncoder().encode(object), urlResponse: .successful()))
        let networkHelper = NetworkHelper(networkClient: mockClient)
    ...
    func test_getJsonAsObject_with_etag_sends_correct_urlRequest() {
        ...
        let networkHelper = NetworkHelper(networkClient: mockClient)
        _ = networkHelper.getJsonAsObject(url: url,  etag: "some etag") { (result: Result<MockResultObject, NetworkError>) in }
        waitForExpectations(timeout: 2.0)
    }
