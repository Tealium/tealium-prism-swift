//
//  CollectConfigurationTests.swift
//  tealium-swift_Tests
//
...
final class CollectConfigurationTests: XCTestCase {
    let customUrl = "https://www.tealium.com/somePath?somekey=someValue"

    func test_overrideDomainURL_changes_domain_on_baseUrl() {
        let url = CollectConfiguration.overrideDomainURL(configuration: [CollectConfiguration.Keys.overrideDomain: "www.overriddenDomain.com"], baseURL: customUrl)
    ...
    func test_getURL_returns_custom_url_when_provided_in_the_configuration() {
        let url = CollectConfiguration.getURL(configuration: [CollectConfiguration.Keys.url: customUrl])
        XCTAssertEqual(url?.absoluteString, customUrl)
    }

    func test_getURL_returns_default_url_when_not_provided_in_the_configuration() {
        let url = CollectConfiguration.getURL(configuration: [:])
        XCTAssertEqual(url?.absoluteString, CollectConfiguration.Defaults.url)
    }

    func test_getURL_overrides_domain_on_default_url() {
        let url = CollectConfiguration.getURL(configuration: [CollectConfiguration.Keys.overrideDomain: "www.overriddenDomain.com"])
    ...
    func test_getURL_does_not_override_domain_on_custom_url() {
        let url = CollectConfiguration.getURL(configuration: [CollectConfiguration.Keys.url: customUrl, CollectConfiguration.Keys.overrideDomain: "www.overriddenDomain.com"])
        XCTAssertEqual(url?.absoluteString, customUrl)
    }

    func test_getBatchURL_returns_custom_batchUrl_when_provided_in_the_configuration() {
        let url = CollectConfiguration.getBatchURL(configuration: [CollectConfiguration.Keys.batchUrl: customUrl])
        XCTAssertEqual(url?.absoluteString, customUrl)
    }

    func test_getBatchURL_returns_default_batchUrl_when_not_provided_in_the_configuration() {
        let url = CollectConfiguration.getBatchURL(configuration: [:])
        XCTAssertEqual(url?.absoluteString, CollectConfiguration.Defaults.batchUrl)
    }

    func test_getBatchURL_overrides_domain_on_default_batchUrl() {
        let url = CollectConfiguration.getBatchURL(configuration: [CollectConfiguration.Keys.overrideDomain: "www.overriddenDomain.com"])
    ...
    func test_getBatchURL_does_not_override_domain_on_custom_batchUrl() {
        let url = CollectConfiguration.getBatchURL(configuration: [CollectConfiguration.Keys.batchUrl: customUrl, CollectConfiguration.Keys.overrideDomain: "www.overriddenDomain.com"])
        XCTAssertEqual(url?.absoluteString, customUrl)
    }

    func test_overrideProfile_is_set_on_init() {
        let configuration = CollectConfiguration(configuration: [CollectConfiguration.Keys.overrideProfile: "override"])
        XCTAssertEqual(configuration?.overrideProfile, "override")
    }

    func test_is_not_initialized_when_url_is_invalid() {
        XCTAssertNil(CollectConfiguration(configuration: [CollectConfiguration.Keys.url: ""]))
    }

    func test_is_not_initialized_when_batchUrl_is_invalid() {
        XCTAssertNil(CollectConfiguration(configuration: [CollectConfiguration.Keys.batchUrl: ""]))
    }
    ...
    func test_create_configuration_from_builder_sets_url_batchUrl_and_overrideProfile() {
        ...
        let configuration = CollectConfiguration(configuration: moduleSettings?.configuration)
