//
//  DeviceDataCollectorTests.swift
//  tealium-swift
//
...
final class DeviceDataCollectorTests: DeviceDataCollectorBaseTests {
    @StateSubject([:])
    var configuration: ObservableState<DataObject>
    lazy var deviceDataCollector = DeviceDataCollector(deviceDataProvider: DeviceDataProvider(),
                                                       configuration: DeviceDataConfiguration(configuration: configuration.value),
    ...
    func test_the_module_id_is_correct() {
        ...
        XCTAssertNotNil(context.moduleStoreProvider.modulesRepository.getModules()[DeviceDataCollector.id])
    }
    ...
    func test_collect_returns_data_with_memory_usage_when_enabled() {
        _configuration.value = [
            DeviceDataConfiguration.Keys.memoryReportingEnabled: true
        ]
        let collected = deviceDataCollector.collect(dispatchContext).asDictionary()
    ...
    func test_applyTransformation_enriches_dispatch_with_model_info_and_orientation() {
        ...
        let transformation = TransformationSettings(id: "model-info", transformerId: DeviceDataCollector.id, scopes: [.afterCollectors])
    ...
    func test_saveModelInfo_saves_data_to_store() throws {
        let dataStore = try context.moduleStoreProvider.getModuleStore(name: DeviceDataCollector.id)
    ...
    func test_convenience_init_creates_module_with_valid_configuration() {
        let moduleConfiguration: DataObject = [
            DeviceDataConfiguration.Keys.memoryReportingEnabled: true,
            DeviceDataConfiguration.Keys.deviceNamesUrl: "example.com/device-names.json"
        ]
        let module = DeviceDataCollector(context: context, moduleConfiguration: moduleConfiguration)
    ...
    func test_module_registers_transformation_on_init() {
        ...
        XCTAssertEqual(transformerId, DeviceDataCollector.id)
    }
    ...
    func test_configuration_change_doesnt_trigger_resource_refresher_update_if_data_is_present() {
        ...
        let newConfig: DataObject = [DeviceDataConfiguration.Keys.deviceNamesUrl: "https://test.url"]
    ...
    func test_instantiating_another_instance_with_same_configuration_gets_cached_models_data() {
        ...
        networkHelper.requests.subscribeOnce { request in
            ...
            if url.absoluteString == DeviceDataConfiguration.Defaults.deviceNamesUrl {
                modelsRequestSent.fulfill()
            }
        }
        _ = DeviceDataCollector(deviceDataProvider: DeviceDataProvider(),
                                configuration: DeviceDataConfiguration(configuration: [:]),
                                networkHelper: networkHelper,
                                storeProvider: context.moduleStoreProvider,
                                transformerRegistry: transformerRegistry,
                                logger: context.logger)
    ...
    func test_default_model_info_is_added_on_apply_transformation_when_no_info_downloaded() {
        let transformationExpectation = expectation(description: "Transformation completed")
        let dispatch = Dispatch(name: "test_event", data: [:])
        let transformation = TransformationSettings(id: "model-info-and-orientation", transformerId: DeviceDataCollector.id, scopes: [.afterCollectors])
        _configuration.value = [
            DeviceDataConfiguration.Keys.deviceNamesUrl: ""
        ]
        deviceDataCollector.applyTransformation(transformation, to: dispatch, scope: .afterCollectors) { result in
            guard let result else {
                XCTFail("Transformation result should not be nil")
