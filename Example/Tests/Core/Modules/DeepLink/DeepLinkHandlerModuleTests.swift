//
//  DeepLinkHandlerModuleTests.swift
//  tealium-swift
//
...
final class DeepLinkHandlerModuleTests: DeepLinkHandlerBaseTests {
    func test_the_module_id_is_correct() {
        XCTAssertNotNil(context().moduleStoreProvider.modulesRepository.getModules()[DeepLinkHandlerModule.id])
    }
    ...
    func test_collect_returns_empty_data_object_when_source_is_deep_link_handler() throws {
        let link = try "https://tealium.com?tealium_trace_id=\(testTraceId)".asUrl()
        try deepLink.handle(link: link)
        XCTAssertEqual(deepLink.collect(DispatchContext(source: .module(DeepLinkHandlerModule.self), initialData: [:])).count, 0)
    }
}
