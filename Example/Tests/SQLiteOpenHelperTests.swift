...
class SQLiteOpenHelperTests: XCTestCase {
    ...
    var testCoreSettings: CoreSettings?
    
    override func setUp() {
        testCoreSettings = CoreSettings(coreDictionary: ["account": "test", "profile": "test"])
    ...
    func test_prepare_runs_all_functions_in_order_for_create() {
        ...
        let mockDatabaseHelper = try? MockDatabaseHelper(databaseName: "mock_database", version: 2,
           onCreateCallback: { database in
                createExpectation.fulfill()
           },
           onConfigureCallback: { database in
                configureExpectation.fulfill()
           },
           onOpenCallback: { database in
    ...
    func test_prepare_runs_all_functions_in_order_for_upgrade() {
        ...
        let onOpenExpectation = expectation(description: "onOpen is called.")
        let mockDatabaseHelper = try? MockDatabaseHelper(databaseName: "mock_database", version: 2,
         onUpgradeCallback: { database, newVersion, oldVersion in
    ...
    func test_prepare_runs_all_functions_in_order_for_downgrade() {
        ...
        mockDatabaseHelper = try? MockDatabaseHelper(databaseName: "mock_database", version: 1,
           onDowngradeCallback: { database, newVersion, oldVersion in
    ...
    func test_onCreate_called_when_dowgrade_throws_UnsupportedDowgrade() {
        ...
        mockDatabase = try? MockDatabaseHelper(databaseName: "mock_database", version: 1,
        onDowngradeCallback: { database, newVersion, oldVersion in
    ...
    func test_onCreate_throws_returns_nil() {
        let mockDatabase = try? MockDatabaseHelper(databaseName: "mock_database", version: 1, onCreateCallback: { connection in
    ...
    func test_onUpgrade_gets_called() {
        ...
        _ = try? MockDatabaseHelper(databaseName: "database_to_upgrade", version: 1, onCreateCallback: { database in
            onCreateCalled.fulfill()
        })
        let mockDatabaseHelper =  try? MockDatabaseHelper(databaseName: "database_to_upgrade", version: 2, onUpgradeCallback: { database, oldVersion, newVersion in
            if (newVersion == 2) {
    ...
    func test_onDowngrade_gets_called() {
        ...
        _ = try? MockDatabaseHelper(databaseName: "database_to_downgrade", version: 1, onCreateCallback: { database in
            onCreateCalled.fulfill()
        })
        _ = try? MockDatabaseHelper(databaseName: "database_to_downgrade", version: 2, onUpgradeCallback: { database, oldVersion, newVersion in
            if (newVersion == 2) {
                onUpgradeCalled.fulfill()
            }
        })
        let mockDatabaseHelper = try? MockDatabaseHelper(databaseName: "database_to_downgrade", version: 1, onDowngradeCallback: { database, oldVersion, newVersion in
    ...
    func test_prepare_throws_when_onCreate_throws() {
        ...
        mockDatabaseHelper.onCreateCallback = { db in
    ...
    func test_prepare_throws_when_onConfigure_throws() {
        ...
        mockDatabaseHelper.onConfigureCallback = { db in
            throw NSError(domain: "", code: 1)
        }
        mockDatabaseHelper.getDatabase()?.userVersion = UserVersion(Int(0))
        XCTAssertThrowsError(try mockDatabaseHelper.prepare())
    ...
    func test_prepare_throws_when_onUpgrade_throws() {
        ...
        mockDatabaseHelper.onUpgradeCallback = { db, newVersion, oldVersion in
            throw NSError(domain: "", code: 1)
        }
    ...
    func test_prepare_throws_when_onDowngrade_throws() {
        ...
        mockDatabaseHelper.onDowngradeCallback = { db, newVersion, oldVersion in
            throw NSError(domain: "", code: 1)
        }
