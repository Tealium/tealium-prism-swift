//
//  LaunchSleepLaunchInAMonthTests.swift
//  LifecycleTests_iOS
//
...
final class LaunchSleepLaunchInAMonthTests: LifecycleServiceBaseTests {
    ...
    var secondLaunchTimestamp: Int64 { sleepTimestamp + (30 * millisecondsPerDay) + millisecondsPerHour }
    var secondLaunchDateString: String { Date(unixMilliseconds: secondLaunchTimestamp).iso8601String }
    var customEventTimestamp: Int64 { secondLaunchTimestamp + millisecondsPerHour }
    ...
    func test_didDetectCrash_is_only_defined_on_launch_events_after_a_crash() {
    ...
    func test_daysSinceLaunch_is_the_difference_between_event_dates_and_launch_date_in_days() {
        XCTAssertEqual(lifecycleEventState.getDataItem(key: LifecycleStateKey.daysSinceFirstLaunch)?.get(), 31)
        XCTAssertEqual(customEventState.getDataItem(key: LifecycleStateKey.daysSinceFirstLaunch)?.get(), 31)
    }
    ...
    func test_daysSinceLastWake_is_the_difference_between_event_dates_and_last_launch_or_wake_date_in_days() {
        XCTAssertEqual(lifecycleEventState.getDataItem(key: LifecycleStateKey.daysSinceLastWake)?.get(), 31)
    ...
    func test_secondsAwake_is_aggregate_seconds_app_was_in_foreground_until_current_event_during_this_session() {
        ...
        XCTAssertEqual(customEventState.getDataItem(key: LifecycleStateKey.secondsAwake)?.get(), secondsPerHour)
    }
