...
final class LaunchSleepTests: LifecycleServiceBaseTests {
    var sleepTimestamp: Int64 { launchTimestamp + millisecondsPerDay }
    var customEventTimestamp: Int64 { sleepTimestamp + millisecondsPerDay }
    ...
    override func setUpWithError() throws {
        ...
        lifecycleEventState = try lifecycleService.registerSleep(timestamp: sleepTimestamp)
    ...
    func test_dayOfWeekLocal_is_correct() {
        XCTAssertEqual(lifecycleEventState.getDataItem(key: LifecycleStateKey.dayOfWeekLocal)?.get(), lifecycleService.calendar
            .component(.weekday, from: Date(unixMilliseconds: sleepTimestamp)))
    ...
    func test_daysSinceLaunch_is_correct() {
        XCTAssertEqual(lifecycleEventState.getDataItem(key: LifecycleStateKey.daysSinceFirstLaunch)?.get(), 1)
        XCTAssertEqual(customEventState.getDataItem(key: LifecycleStateKey.daysSinceFirstLaunch)?.get(), 2)
    }
    ...
    func test_daysSinceLastWake_is_correct() {
        XCTAssertEqual(lifecycleEventState.getDataItem(key: LifecycleStateKey.daysSinceLastWake)?.get(), 1)
        XCTAssertEqual(customEventState.getDataItem(key: LifecycleStateKey.daysSinceLastWake)?.get(), 2)
    }
    ...
    func test_hourOfDayLocal_is_correct() {
        XCTAssertEqual(lifecycleEventState.getDataItem(key: LifecycleStateKey.hourOfDayLocal)?.get(), lifecycleService.calendar
            .component(.hour, from: Date(unixMilliseconds: sleepTimestamp)))
