...
class VisitorSwitcher {
    /**
     * Subscribes the given `visitorIdStorage` to receive identity updates from the lastest
     ...
     * then the value will be taken as the identity and the `visitorIdStorage` will be notified.
     *
     ...
     *
     * - Parameters:
     *   - visitorIdStorage: The `VisitorIdStorage` to notify of identity updates.
     ...
     */
    static func handleIdentitySwitches(visitorIdStorage: VisitorIdStorage, onCoreSettings: Observable<CoreSettings>, dataLayerStore: any DataStore) -> any Disposable {
        return onCoreSettings.map { $0.visitorIdentityKey }
            ...
            .subscribe { identity in
                visitorIdStorage.identify(identity: identity)
            }
    }
}
