...
typealias ScopedBarrier = (barrier: Barrier, scope: BarrierScope)
 ...
 *
 * It exposes one `Observable` for all of the barriers along with their specific scope, merging both of the arrays it manages.
 */
class BarrierManager: BarrierRegistrar {
    ...
    private var defaultBarrierScope: [String: BarrierScope] = [:]

    /// An `Observable` for the configuration and registered barriers, along with their specific scope.
    ...
    func initializeBarriers(factories: [any BarrierFactory], context: TealiumContext) {
        defaultBarrierScope = factories.reduce(into: [:]) { partialResult, factory in
            partialResult[factory.id] = factory.defaultScope()
        }
    ...
    /// An observable for the configuration barriers along with their scope, updated every time new settings arrive.
    func scopedConfigBarriers() -> Observable<[ScopedBarrier]> {
        configBarriers.combineLatest(sdkBarrierSettings)
            .map { [weak self] barriers, barrierSettings in
                barriers.map {
                    (barrier: $0,
                     scope: barrierSettings[$0.id]?.scope ?? self?.defaultBarrierScope[$0.id] ?? .all)
                }
            }
    }

    /// Register a barrier to a specific scope, overriding the scope in case the same barrier was already added.
    ...
    func registerScopedBarrier(_ barrier: any Barrier, scope: BarrierScope) {
        let scopedBarrier: ScopedBarrier = (barrier, scope)
